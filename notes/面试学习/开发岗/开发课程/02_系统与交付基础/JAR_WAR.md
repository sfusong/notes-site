...
`Spring Boot 可以打 jar，也可以打 war，只是大多数新项目更常见 jar。`

通常只有这些场景更可能打成 `war`：

- 公司历史架构要求统一部署到外部 Tomcat
- 运维规范要求使用统一应用服务器
- 老系统迁移时需要保持原有部署方式

面试里如果被追问，可以这样说：

“Spring Boot 不是不能打 war，只是它默认更鼓励打成可执行 jar。因为 jar 可以把内嵌 Tomcat 和应用一起打包，直接 `java -jar` 启动，部署更简单，也更适合自动化交付和容器化。如果公司有统一外部 Tomcat 或老系统规范，也可以打成 war 部署。”
